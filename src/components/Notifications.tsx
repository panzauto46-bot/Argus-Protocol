    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8" data-reveal>
        ...
        <p className={`text-sm mt-1 ${dark ? "text-gray-400" : "text-gray-500"}`}>
          Connect messaging channels and inspect alerts pushed from live Reactivity stream
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className={`p-6 rounded-2xl border ${dark ? "bg-argus-card/50 border-argus-border" : "bg-white border-gray-200"}`} data-reveal data-reveal-delay={80} data-tilt data-pop="strong" data-spotlight data-tilt-strength={5}>
          ...
          <div className="space-y-4">
            <div>
              <label className={`text-xs font-medium mb-1.5 block ${dark ? "text-gray-400" : "text-gray-500"}`}>Bot Token</label>
              <div className="relative">
            ...
            <div>
              <label className={`text-xs font-medium mb-1.5 block ${dark ? "text-gray-400" : "text-gray-500"}`}>Chat ID</label>
              <div className="relative">
                ...
                <input
                  ...
                  value={telegramChat}
                  onChange={(e) => {
                    setTelegramChat(e.target.value);
                    setTelegramSuccess(false);
                  }}
      ...
      <div className={`rounded-2xl border ${dark ? "bg-argus-card/50 border-argus-border" : "bg-white border-gray-200"}`} data-reveal data-reveal-delay={120} data-tilt data-pop="strong" data-spotlight data-tilt-strength={4.6}>
        <div className="p-6 pb-4">
          ...
          <p className={`text-xs ${dark ? "text-gray-500" : "text-gray-400"}`}>Global alert feed from wallet, config, and reactivity stream</p>
        </div>
        <div className="divide-y divide-argus-border">
          {alerts.map((notif) => {
            const config = typeConfig[notif.level] || typeConfig.info;
            return (
              <div
                key={notif.id}
                data-reveal="left"
