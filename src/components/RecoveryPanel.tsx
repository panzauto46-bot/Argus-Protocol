...
export default function RecoveryPanel() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Resolve & Unpause */}
      {isPaused && !resolved && (
        <div className={`rounded-2xl border-2 p-6 ${dark ? 'border-red-500/30 bg-red-500/5' : 'border-red-300 bg-red-50'}`} data-reveal data-reveal-delay={120}>
            ...
            <div className="space-y-4">
              <div className={`p-4 rounded-xl border ${dark ? 'bg-yellow-500/5 border-yellow-500/20' : 'bg-yellow-50 border-yellow-200'}`}>
                <p className={`text-sm ${dark ? 'text-yellow-300' : 'text-yellow-700'}`}>
                  <strong>⚠️ Warning:</strong> You are about to unpause the contract. Make sure the exploit has been addressed and the attacker wallet has been blacklisted.
                </p>
              </div>
